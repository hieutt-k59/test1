...
namespace RBI.BUS.BUSMSSQL_CAL
{
    class MSSQL_DM_CAL
    {
        ...
        public float Art(float age)
        {
            ...
            if (!InternalCladding)
            {
                art = Math.Max(1 - (CurrentThick - CorrosionRate * age) / (getTmin() + CA), 0);
                Console.WriteLine("Art: " + art);
                return art;
            }
            else
            {
                art = 1 - (CurrentThick - CladdingCorrosionRate * agerc() - CorrosionRate * (age - agerc())) / (getTmin() + CA);
                Console.WriteLine("Art: " + art);
                return Math.Max(art, 0);
            }
        }
        ...
        public int DFB_THIN(float age)
        {
            ...
            else
            {
                ...
                {
                    Console.WriteLine("Num of Insp: " + NoINSP_THINNING + EFF_THIN);
                    Console.WriteLine("Art api: " + API_ART(Art(age)));
                    Console.WriteLine("DFb_thin: " + DAL_CAL.GET_TBL_511(API_ART(Art(age)), NoINSP_THINNING, EFF_THIN));
        ...
        public float DF_THIN(float age)
        {
            ...
            int dfb_thin = DFB_THIN(age);
            return dfb_thin * Fip * Fdl * Fwd * Fsm * Fam / Fom;
        }
